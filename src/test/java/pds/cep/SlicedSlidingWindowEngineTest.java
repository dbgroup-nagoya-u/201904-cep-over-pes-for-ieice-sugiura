 ...
 */
class SlicedSlidingWindowEngineTest {
  ...
  public void testRun() {
    ...
    SlicedSlidingWndowEngine engine = new SlicedSlidingWndowEngine(stream, dfa, 3, 2);
    engine.run();
    assertEquals("[0 : 2]: 0.09000\n", out.toString());

    out.reset();
    stream = new SampleStream(8);
    engine = new SlicedSlidingWndowEngine(stream, dfa, 4, 3);
    engine.run();
